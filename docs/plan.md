...
| X | **SQL og databaseadgang:** al dataadgang går gennem EF Core/LINQ med parametre. Den eneste SQL-tekst er de faste tabel-låse (`TableLock`, en enum), og rå SQL i API'et fanges af en test, af analyzere i bygget og af CodeQL i CI. I driften får appen mindste rettighed: `ea_app` kun DML i skemaet `ea` (og ingen UPDATE/DELETE på historikken, W); migreringer kører som `ea_migrator`, der ejer skemaet; tjekket af ventende migreringer bruger en læserolle. Rollerne oprettes af ejerens script i F3/F4. Lokalt bruges én rolle (`ea`), fordi testene opretter databaser. | Et fremtidigt kald med en dynamisk streng må ikke kunne slippe igennem ubemærket. Mindste rettighed begrænser skaden, hvis noget alligevel gør (ejeren, 2026-09-25). |
